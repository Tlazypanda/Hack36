      ...
      <div class="modal-wrapper">
        <div class="modal-container">
          <div class="modal-header">
            <h3>Presciption Details</h3>
          </div>
          <div class="modal-body">
            <slot name="body">
              <h3>Title: {{propData.patientHealthCardNumber}}</h3>
              <h3>Price: {{propData.price}}</h3>
              <p>Description: {{propData.drugName}}</p>
              <div class="row ml-4 p-2">
                <datepicker :value="startDate" v-model="startDate"></datepicker>
                <datepicker v-model="endDate"></datepicker>
              </div>
              <b-button v-on:click="order" class="mr-5 mt-3">
                <span>Order Now</span>
              </b-button>
            </slot>
          </div>
...
export default {
  ...
  // price calculation
  const totalPrice = web3().utils.toWei(this.propData.price, 'ether')
